...
void micro_tk(const micro_globals g) {
    ...
    st_fp8e4m3<BLOCK_SIZE, K_STEP> (&As) = al.allocate<st_fp8e4m3<BLOCK_SIZE, K_STEP>>();
    st_fp8e4m3<BLOCK_SIZE, K_STEP> (&Bs) = al.allocate<st_fp8e4m3<BLOCK_SIZE, K_STEP>>();

    rt_fp8e4m3<REG_BLOCK, DOT_SLICE> tiles[8];
    ...
    for (int tile = 0; tile < num_tiles - 1; ++tile) {
        ...
        float4 a_buffer_next[BUFFER_SIZE * sizeof(fp8e4m3) / sizeof(float4)];
        float4 b_buffer_next[BUFFER_SIZE * sizeof(fp8e4m3) / sizeof(float4)];
